...
void ClusterNode::connectToMasterIfNeeded()
{
    ...
    const auto registration = encodeArray({"REPLICA_REGISTER",
                                           endpoint().host,
...
{
    const auto frame = encodeArray({"HEARTBEAT",
...
bool ClusterNode::sendVoteRequest(const Endpoint &peer, const std::uint64_t term, const std::uint64_t dispatch_progress)
{
    ...
    const auto request = encodeArray({"VOTE_REQUEST",
                                      std::to_string(term),
                                      endpoint().host,
                                      std::to_string(endpoint().port),
...
void ClusterNode::broadcastNewMaster()
{
    ...
    const auto frame = encodeArray({"NEW_MASTER",
                                    std::to_string(current_term_.load()),
                                    endpoint().host,
                                    std::to_string(endpoint().port)});
